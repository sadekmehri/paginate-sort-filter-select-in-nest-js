...
export class PaginationService<T extends object, D> {
  private readonly paginationServiceStrategy: IPaginationStrategy<D>

  constructor(paginationServiceStrategy: IPaginationStrategy<D>) {
  ...
  async processPagination(request: Request, queryArgs: T) {
    ...
    const result: PaginatedResult<D> = {
      data,
      meta: {
        total: nbrRecords,
        perPage: limit,
